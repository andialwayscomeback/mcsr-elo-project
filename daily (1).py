...
boundary. Does two things:
  ...
  2. Looks back over the last 24 hours and computes upset candidates among
     THIS roster (today's top N, not yesterday's) -- so a player who
     climbed into the top N partway through the day is still included,
     since all that matters is where they ended up. Writes a
     human-readable summary to candidates/YYYY-MM-DD.md (covering the
     previous 24 hours) plus the raw data to candidates/YYYY-MM-DD.json.
    ...
    for p in snapshot["players"]:
        for m in fetch_player_matches_in_window(p["uuid"], since_ts, until_ts):
            ...
            if len(player_uuids) != 2 or not all(u in rank_by_uuid for u in player_uuids):
                continue  # one player isn't in today's top N
